...
public class ControllerTemplateGenerate {

    private ClassInfo classInfo;

    private MetaDataInfo metaDataInfo;

    public ControllerTemplateGenerate(ClassInfo classInfo, MetaDataInfo metaDataInfo) {
    ...
    public void generate() throws IOException, TemplateException {
        ...
        Writer out = null;
        try {
            String controllerName = classInfo.getClassSimpleName() + "Controller.java";
            Template template = FreeMarkerTemplate.getTemplate("Controller.ftl");
            File dir = new File(classInfo.getSaveDirPath(), "/controller");
            if (!dir.exists()) {
                if (!dir.mkdirs()) {
                    throw new RuntimeException("创建文件夹失败:" + dir.getAbsolutePath());
                }
            }
            File daoFile = new File(dir, controllerName);
            ...
            System.out.println("ControllerFile:" + daoFile.getAbsoluteFile());
